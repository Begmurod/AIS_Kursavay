...
{
    public partial class materialUC : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        public materialUC(Control _parent)
        {
            InitializeComponent();
            Parent = _parent;
  ...
  WHERE [DeletedDate] is null"
        ...
        {
            using (MaterialForm addForm = new MaterialForm())
            {
                addForm.ShowDialog();
                addForm.Dispose();
            }
        ...
        private void barButtonItemView_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            viewForm.currentMaterialGuid = (Guid)materialGridView.GetFocusedRowCellValue("GUID");
            viewForm.edit = true;
            viewForm.view = true;
            viewForm.ShowDialog();
        }
        ...
        {
            using (MaterialForm editForm = new MaterialForm())
            {
                editForm.currentMaterialGuid = (Guid)materialGridView.GetFocusedRowCellValue("GUID");
                editForm.edit = true;
                editForm.ShowDialog();
            }
            loadData();
        ...
        private void barButtonItemDelete_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            using (UnitOfWork u = new UnitOfWork())
            {
                device_Material currentMaterial = u.GetObjectByKey<device_Material>((Guid)materialGridView.GetFocusedRowCellValue("GUID"));
                DialogResult d = XtraMessageBox.Show(string.Format("Удалить тип ремонта {0}?", currentMaterial.Name), "Подтверждение действия", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                if (d == DialogResult.Yes)
                {
                    currentMaterial.DeletedDate = DateTime.Now;
        ...
        private void materialGridView_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
